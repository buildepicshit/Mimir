...
This specification defines the contract of `mimir-cli`, the read-only inspection tool that humans and audit scripts use to observe Mimir's canonical state. Per AGENTS.md invariant #2, the canonical form is not human-readable — inspection always routes through this tool. The decoder is the sanctioned bridge between the agent-native binary format and human / external consumers.
