...
- `crate::LlmInvoker` — trait over "ask a selected agent adapter to structure this prose as canonical Lisp." Production impls include `ClaudeCliInvoker`, `CodexCliInvoker`, and `CopilotCliInvoker`; tests mock via any `LlmInvoker` implementation.
...
- **LLM invocation:** native agent subprocesses from Rust, wrapped behind a `LlmInvoker` trait so tests can mock. Claude uses `claude -p`; Codex uses `codex exec --output-last-message`; Copilot defaults to `gh copilot -p`. No direct vendor SDK dependency.
...
- [x] PR: `LlmInvoker::invoke` with real Claude, Codex, and Copilot subprocess handling (timeout, stderr capture, typed errors on non-zero exit or non-JSON response).
